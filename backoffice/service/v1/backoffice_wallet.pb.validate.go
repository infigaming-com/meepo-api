...
func (m *GetExchangeRatesResponse) validate(all bool) error {
	...
	{
		sorted_keys := make([]string, len(m.GetRatesWithFee()))
		i := 0
		for key := range m.GetRatesWithFee() {
			sorted_keys[i] = key
			i++
		}
		sort.Slice(sorted_keys, func(i, j int) bool { return sorted_keys[i] < sorted_keys[j] })
		for _, key := range sorted_keys {
			val := m.GetRatesWithFee()[key]
			_ = val

			// no validation rules for RatesWithFee[key]

			if all {
				switch v := interface{}(val).(type) {
				case interface{ ValidateAll() error }:
					if err := v.ValidateAll(); err != nil {
						errors = append(errors, GetExchangeRatesResponseValidationError{
							field:  fmt.Sprintf("RatesWithFee[%v]", key),
							reason: "embedded message failed validation",
							cause:  err,
						})
					}
...
// Validate checks the field values on RateWithFee with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *RateWithFee) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on RateWithFee with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in RateWithFeeMultiError, or
// nil if none found.
func (m *RateWithFee) ValidateAll() error {
	return m.validate(true)
}

func (m *RateWithFee) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	// no validation rules for FeePercentage

	if len(errors) > 0 {
		return RateWithFeeMultiError(errors)
	}

	return nil
}

// RateWithFeeMultiError is an error wrapping multiple validation errors
// returned by RateWithFee.ValidateAll() if the designated constraints aren't met.
type RateWithFeeMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m RateWithFeeMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m RateWithFeeMultiError) AllErrors() []error { return m }

// RateWithFeeValidationError is the validation error returned by
// RateWithFee.Validate if the designated constraints aren't met.
type RateWithFeeValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e RateWithFeeValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e RateWithFeeValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e RateWithFeeValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e RateWithFeeValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e RateWithFeeValidationError) ErrorName() string { return "RateWithFeeValidationError" }

// Error satisfies the builtin error interface
func (e RateWithFeeValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = RateWithFeeValidationError{}
